...
$updateDataRaw = [System.IO.File]::ReadAllText($JsonFile, [System.Text.Encoding]::UTF8)
$updateData = ConvertFrom-Json $updateDataRaw

$updatedCount = 0

foreach ($p in $packs) {
    if ($updateData.PSObject.Properties.Match($p.id).Count -gt 0) {
        $updateInfo = $updateData.$($p.id)
...
Write-Output "Successfully updated $updatedCount packs from $JsonFile"